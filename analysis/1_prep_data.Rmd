...
```{r}
fig_11 <- df %>% select("ID","fear.longterm", "fear.admission", "fear.effectiveness", "fear.side.effects", "fear.no.danger") %>%
mutate_if(is.character,as.numeric) %>% 
  dplyr::rename("Longterm Consequences"  = fear.longterm,  
                "Doubt Admission"  = fear.admission, 
                "Effectiveness" = fear.effectiveness, 
                "Side Effects" = fear.side.effects, 
                "Covid not dangerous" = fear.no.danger)%>% 
  melt( id.vars = c("ID"), variable.name = "Group") %>% 
  drop_na() %>%
  dplyr::filter(value == "1")%>%
  group_by(Group) %>% 
  mutate(count = n()) %>% 
  distinct(Group, count, .keep_all = TRUE )%>% 
  group_by(value) %>%
  mutate(sum = sum(count)) %>% 
  mutate(perc = (count / sum)*100) %>% 
  ggplot() +
  geom_bar(aes(x = reorder(Group, +perc), y = perc), stat = "identity", width = 0.6,alpha = 0.5)+
  theme_bw(base_size=18)+
  ylab("Percent (%)")+
  labs(title = "Vaccination Fear")+
  theme(axis.title.y = element_blank())+
  coord_flip()
 fig_11
 
pdf(fig_11_path, height = 3, width = 6)
fig_11
dev.off()
